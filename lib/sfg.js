/**
 * ForumIAS SFG 50-Q Converter — Node.js version
 * Exact same parsing logic as the HTML version.
 */

const { getDocument, GlobalWorkerOptions } = require('pdfjs-dist');

// IMPORTANT: In Vercel serverless, pdf.js can't find './pdf.worker.js' locally.
// Same fix as the HTML version — set workerSrc to the CDN URL.
// This MUST be set BEFORE any getDocument() calls.
GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';

function extractPageText(content) {
  if (!content.items || !content.items.length) return '';
...
async function processSfg(buffer, onProgress) {
  try {
    ...
    // Node.js Buffer → plain Uint8Array (pdfjs rejects Buffer)
